...
//					} 
		...
		case Test:
			if(MainEditor.getSheet() != null){
				byte[] data = MainEditor.getSheet().getPlateauBytes();
				MainEditor.getSheet().undoLastAction();
				MainEditor.getSheet().redoNextAction();
				System.out.println(Arrays.equals(data, MainEditor.getSheet().getPlateauBytes()));
			}
			break;
		case ExitSoftware:
			MainEditor.frame.setVisible(false);
