...
//					g.drawImage(c0.image, x+25f, y+40f, x+this.sizeX/3f-25f, y+sizeY*2f/3f,0f,0f,c0.image.getWidth()/3f,c0.image.getHeight()/4f);
				...
				g.fillRect(x+10f, y+this.sizeY-25f, (this.sizeX/3f-20f)*c0.lifePoints/c0.getAttribut(Attributs.maxLifepoints), 20f);
				g.setColor(Color.white);
				String s = "HP: " + (int)c0.lifePoints + " / " + (int)c0.getAttribut(Attributs.maxLifepoints);
				g.drawString(s, x+sizeX/3f+10f, y+10f);
				
				if(c0.getAttribut(Attributs.armor)!=0f)
					s = "Armor: " + (int)c0.getAttribut(Attributs.armor)*10;
				else
					s = "No Armor";
				g.drawString(s, x+sizeX/3f+10f, y+45f);
				//s = "Velocity: " + (int)c0.maxVelocity;
				s = "Speed: " + (int)c0.getAttribut(Attributs.maxVelocity);
				g.drawString(s, x+sizeX/3f+10f, y+80f);
				if(c0.getAttributString(Attributs.weapon)!=null){
					float aux = Main.framerate/(c0.getAttribut(Attributs.chargeTime)*10f);
					int a = (int)(aux*100f)/100;
					int b = (int)(aux*100f)-a*100;
					s = "Att.Rate: " + a+"."+b;
					g.drawString(s, x+2f*sizeX/3f, y+10f);
					float aux1 = Math.abs(c0.getAttribut(Attributs.damage));
					a = (int)(aux1*100f)/100;
					b = (int)(aux1*100f)-a*100;
					if(c0.getAttribut(Attributs.damage)<0)
						s = "Healing: ";
					else
						s = "Damage: "; 
					s+= a +"." +b;
