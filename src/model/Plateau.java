...
public class Plateau {
	...
	public OutputModel update(Input i){
		...
		this.updateView(i, player);
		// Handling groups of units
		for(int to=0; to<10; to++){
			if(i.isKeyPressed(to+2)){
				...
				} else if(i.isKeyPressed(Input.KEY_LSHIFT)){
		...
		//Top Bar
		if((i.getAbsoluteMouseY())<this.g.relativeHeightTopBar*this.g.resY){


		}

		//Bottom Bar
		else if((i.getAbsoluteMouseY())>this.g.players.get(player).bottomBar.y){
			BottomBar bb = this.g.players.get(player).bottomBar;
			float relativeXMouse = (i.getAbsoluteMouseX());
			float relativeYMouse = (i.getAbsoluteMouseY());
		...
		// FIELD
		else if( (i.getAbsoluteMouseY())>=this.g.players.get(player).topBar.y && (i.getAbsoluteMouseY())<=this.g.players.get(player).bottomBar.y ){
			...
			if(i.isMousePressed(Input.MOUSE_LEFT_BUTTON)){
				if(isCastingSpell.get(player)){
					...
					if(this.g.players.get(player).selection.size()>0){
						...
						spell.launch(new Checkpoint(i.getAbsoluteMouseX()+Xcam,i.getAbsoluteMouseY()+Ycam),(Character)this.g.players.get(player).selection.get(0));
			...
			if(i.isMousePressed(Input.MOUSE_RIGHT_BUTTON)){
				//RALLY POINT
				if(this.selection.get(player).size()>0 && this.selection.get(player).get(0) instanceof BuildingProduction){
					((BuildingProduction) this.selection.get(player).get(0)).rallyPoint = new Checkpoint(i.getAbsoluteMouseX()+Xcam,i.getAbsoluteMouseY()+Ycam);
				} else if(isCastingSpell.get(player)){
					isCastingSpell.set(player,false);
					castingSpell.set(player,-1);
				} else if(i.isKeyPressed(Input.KEY_LSHIFT)){
					updateSecondaryTarget(i.getAbsoluteMouseX()+Xcam,i.getAbsoluteMouseY()+Ycam,player);
				} else {				
					updateTarget(i.getAbsoluteMouseX()+Xcam,i.getAbsoluteMouseY()+Ycam,player);
				}
			}
		}
		// Handling other hotkeys in production bar
		if(i.isKeyPressed(Input.KEY_W) || i.isKeyPressed(Input.KEY_X) || i.isKeyPressed(Input.KEY_C) || i.isKeyPressed(Input.KEY_V) || i.isKeyPressed(Input.KEY_ESCAPE)){
			if(this.selection.get(player).size()>0 && this.selection.get(player).get(0) instanceof BuildingProduction){
	...
	public void updateView(Input i, int player){
		if(!isCastingSpell.get(player) && player==this.g.currentPlayer && this.rectangleSelection.get(player)==null && !i.isMousePressed(Input.MOUSE_LEFT_BUTTON)){
			// Move camera according to inputs :
			if((i.isKeyDown(Input.KEY_UP) || i.getAbsoluteMouseY()<10)&&this.Ycam>-this.g.resY/2){
				Ycam -= 20;
			}
			if((i.isKeyDown(Input.KEY_DOWN) || i.getAbsoluteMouseY()>this.g.resY-10) && this.Ycam<this.maxY-this.g.resY/2){
				Ycam +=20;
			}
			if((i.isKeyDown(Input.KEY_LEFT)|| i.getAbsoluteMouseX()<+10) && this.Xcam>-this.g.resX/2 ){
				Xcam -=20;
			}
			if((i.isKeyDown(Input.KEY_RIGHT) || i.getAbsoluteMouseX()>this.g.resX-10)&& this.Xcam<this.maxX-this.g.resX/2){
				Xcam += 20;
			}
			...
			for(int to=0; to<10; to++){
				if(i.isKeyPressed(to+2)){
					if(this.g.players.get(player).groupSelection == to && this.g.players.get(player).groups.get(to).size()>0){
						...
						this.Xcam = Math.min(maxX-this.g.resX/2f, Math.max(-this.g.resX/2f, xmoy-this.g.resX/2f));
						this.Ycam = Math.min(maxY-this.g.resY/2f, Math.max(-this.g.resY/2f, ymoy-this.g.resY/2f));
					}
				}
			}
		}
		...
		// check if in topbar
		if((i.isMousePressed(Input.MOUSE_LEFT_BUTTON)||i.isMousePressed(Input.MOUSE_RIGHT_BUTTON)) && (i.getAbsoluteMouseY())<bottomTopBar){
			if(this.rectangleSelection.get(player)!=null){
				rectangleSelection.get(player).setBounds( (float)Math.min(recX.get(player),i.getAbsoluteMouseX()+Xcam), (float)this.g.players.get(player).topBar.y
						+(float)this.g.players.get(player).topBar.sizeY+this.Ycam+2f,
						(float)Math.abs(i.getAbsoluteMouseX()+Xcam-recX.get(player))+0.1f, (float)Math.abs(this.g.players.get(player).topBar.y+(float)this.g.players.get(player).topBar.sizeY+2f+this.Ycam-recY.get(player))+0.1f);
			}
		} else 
			// check if in bottombar	
			if((i.isMousePressed(Input.MOUSE_LEFT_BUTTON)||i.isMousePressed(Input.MOUSE_RIGHT_BUTTON)) && (i.getAbsoluteMouseY())>this.g.players.get(player).bottomBar.y){

				BottomBar b = this.g.players.get(player).bottomBar;
				//If click on minimap
				if(i.isMousePressed(Input.MOUSE_LEFT_BUTTON) && player==this.g.currentPlayer && (i.getAbsoluteMouseX())>b.startX && (i.getAbsoluteMouseX())<
						b.startX+b.w && this.rectangleSelection.get(player)==null){
					// Put camera where the click happened
					Xcam = (int)Math.floor((i.getAbsoluteMouseX()-b.startX)/b.rw)-this.g.resX/2f;
					Ycam = (int)Math.floor((i.getAbsoluteMouseY()-b.startY)/b.rh)-this.g.resY/2f;

				}
				if(i.isMousePressed(Input.MOUSE_RIGHT_BUTTON) && player==this.g.currentPlayer && (i.getAbsoluteMouseX())>b.startX && (i.getAbsoluteMouseX())<
						b.startX+b.w && this.rectangleSelection.get(player)==null){
					// Handle right click
					if(i.isKeyPressed(Input.KEY_LSHIFT)){
						updateSecondaryTarget((int)Math.floor((i.getAbsoluteMouseX()-b.startX)/b.rw),(int)Math.floor((i.getAbsoluteMouseY()-b.startY)/b.rh),player);
					} else {				
						updateTarget((int)Math.floor((i.getAbsoluteMouseX()-b.startX)/b.rw),(int)Math.floor((i.getAbsoluteMouseY()-b.startY)/b.rh),player);
					}

				}
				if(this.rectangleSelection.get(player)!=null){
					rectangleSelection.get(player).setBounds( (float)Math.min(recX.get(player),i.getAbsoluteMouseX()+Xcam), (float)Math.min(recY.get(player), i.getAbsoluteMouseY()+Ycam),
							(float)Math.abs(i.getAbsoluteMouseX()+Xcam-recX.get(player))+0.1f, (float)Math.abs(this.g.players.get(player).bottomBar.y+this.Ycam-2f-recY.get(player))+0.1f);
				}
			}

	}

	private void updateRectangle(Input i, int player) {
		if(rectangleSelection.get(player)==null || i.isKeyPressed(Input.KEY_LCONTROL)){
			recX.set(player, (float)i.getAbsoluteMouseX()+Xcam);
			recY.set(player, (float)i.getAbsoluteMouseY()+Ycam);
			rectangleSelection.set(player, new Rectangle(recX.get(player),recY.get(player),0.1f,0.1f));
		}
		rectangleSelection.get(player).setBounds( (float)Math.min(recX.get(player),i.getAbsoluteMouseX()+Xcam), (float)Math.min(recY.get(player), i.getAbsoluteMouseY()+Ycam),
				(float)Math.abs(i.getAbsoluteMouseX()+Xcam-recX.get(player))+0.1f, (float)Math.abs(i.getAbsoluteMouseY()+Ycam-recY.get(player))+0.1f);
	}
