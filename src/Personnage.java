...
public class Personnage extends Objet{
	...
	public Personnage(Plateau p, float x_origin, float y_origin,float w,float vitesse_attaque,float acc,int camps,int framerate){
		this.camps = camps;
		...
		y_pre = y_origin ;
		this.w = w;
		this.vitesse_attaque = vitesse_attaque/framerate;
		this.attaque_state = 0.99f;
		this.acc = acc;
	...
	public void move(float f, float g){
		...
		box.setLocation(this.x-this.w, this.y-this.w);
	}

	public void tirer(float vxo, float vyo){
		if(this.attaque_state>1f){
		...
		vxo =  vxo/norm;
		vyo =  vyo/norm;
		p.add(new Balle(this,x,y,vxo,vyo,framerate));
		this.attaque_state = 0f;
		}
	}
	...
	public void setXY(float x,float y ) {
		this.x = x;
		this.y = y;
		this.box.setLocation(this.x -this.w,this.y-this.w);
	}
	...
	public void action(Vector<Objet> target) {
		...
		if(this.main_target!=null){
			...
			if(this.main_target.getCamps()!=0 && this.main_target.getCamps()!=this.getCamps()){
				this.tirer(this.main_target.getX(),this.main_target.getY());
			}

		}
		// update the attack state;
		if(this.attaque_state<1f){
		this.attaque_state+=this.vitesse_attaque;
		}
