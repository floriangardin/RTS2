...
class NbObjetsBetween(Feature):
    def __init__(self, objet, to_count, team, low, up):
        quantity = len([val for key, val in objet.plateau.items() if val['name'] == to_count and val['team'] == team])
        self.state = '1' if low <= quantity < up else '0'
...
class EnemyInRangeAtLeast(Feature):
    def __init__(self, objet, to_count, low):
        ...
        enemies = [val for key, val in objet.plateau.items() if val['name'] == to_count and val['team'] != objet.team and val['team'] != 0]
        # Filter in range
